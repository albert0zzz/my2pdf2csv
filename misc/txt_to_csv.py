...
def extract_data_from_text(text: str, file_name:str):
    # Extraction of Ru phone numbers with different formats
    phone_numbers: list[str] = re.findall(r'(\+7|\b8)[\s\(]*(\d{3})[\)]?\s*(\d{3})[-]?\s*(\d{2})[-]?\s*(\d{2})', text) or ['phone']
    ...
    names: list[str] = re.findall(r'\b[А-Я][а-я]+\s[А-Я][а-я]+(?:\s[А-Я][а-я]+)?', text) or ['name']

    # Extracting e-mails 
    emails: list[str] = re.findall(r'\b[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Z|a-z]{2,}\b', text) or ['email']

    # Getting a file name without extension 
    file_name_without_extension: str = os.path.splitext(file_name)[0]
...
def process_files(input_folder: str, output_folder: str):
    csv_file_path: str = os.path.join(output_folder, 'extracted_data.csv')
...
if __name__ == "__main__":
    ...
    input_folder: str = os.path.abspath(os.path.join(script_path, '..', '..', 'txt_files'))
    
    # Absolute address of the "csv_files" folder 
    output_folder: str = os.path.abspath(os.path.join(script_path, '..', '..', 'csv_files'))
